...
public:
    ...
    enum MEHTOD {GET = 0, POST, HEAD, PUT, DELETE, TRACE, OPTIONS, CONNECT, PATCH};//枚举请求方法

	//主状态机的三种可能状态，分别表示：当前正在分析请求行，当前正在分析头部字段，分析数据
 ...
 */
    ...
    enum LINE_STATUS {LINE_OK = 0, LINE_BAD, LINE_OPEN};//从状态机的三种可能状态，即行的读取状态， 分别表示:读取到一个完整的行，行出错，行数据尚且不完整
...
private:
    ...
    //parse http request
    HTTP_CODE process_read();//读取请求方法，做出对应响应
