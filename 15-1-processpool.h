...
public:
    ...
    static processpool<T> *create(int listenfd, int process_number = 8)
    {
        if (!m_instance) 
        {
            m_instance = new processpool<T> (listenfd, process_number);//进行构造函数
        }
        return m_instance;
    }
    ~ processpool()
...
template< typename T>
processpool<T>* processpool<T>::m_instance = NULL;//template< typename T> processpool<T>* 类型，processpool<T>命名空间，m_instance变量名
