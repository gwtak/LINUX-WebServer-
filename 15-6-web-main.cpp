...
int main(int argc, char *argv[])
{
    ...
    const char *ip = argv[1];//得到ip
    int port = atoi(argv[2]);//得到port
    ...
    while(true)
    {
        ...
        for (int i = 0; i < number; i ++)
        {
            ...
            else if (events[i].events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) //链接被对方关闭|挂起|错误
            {
                //关闭连接
            ...
            else if (events[i].events & EPOLLIN) //数据可读
            {
                ...
                {
                    pool->append(users + sockfd);//调用线程池处理
                }
                else
                {
                    users[sockfd].close_conn();
