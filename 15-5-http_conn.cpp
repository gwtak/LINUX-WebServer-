...
{
    int old_option = fcntl(fd, F_GETFL);//文件锁，局部锁定
...
void addfd(int epollfd, int fd, bool one_shot)
{
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLET | EPOLLRDHUP;//可读，边沿触发，挂起(个人推测：按位或，获得所有位上为1的值，使得其具备3个触发条件)
    if(one_shot)
    {
        event.events |= EPOLLONESHOT;//单次触发
    }
    epoll_ctl(epollfd, EPOLL_CTL_ADD, fd, &event);//添加epoll事件
    setnonblocking(fd);
}
//删除epoll事件
...
void http_conn::init(int sockfd, const sockaddr_in &addr)
{
    ...
    getsockopt( m_sockfd, SOL_SOCKET, SO_ERROR, &error, &len );//获取任意类型、任意状态套接口的选项当前值*/
    int reuse = 1;
...
void http_conn::init()
{
    m_check_state = CHECK_STATE_REQUESTLINE;//主状态机状态：分析请求行
    m_linger = false;//请求是否要求保持连接

    m_method = GET;//请求方式为GET
    m_url = 0;//客户请求的目标文件的文件名
    m_version = 0;//HTTP 协议版本号， 我们仅支持HTTP/1.1
    m_content_length = 0;//http请求的消息体的长度
    m_host = 0;//主机名
    m_start_line = 0;//当前正在解析的行的起始位置
    m_checked_idx = 0;//当前正在分析的字符在读缓冲区中的位置
    m_read_idx = 0;//标识读缓冲去已经读入的客户数据的最后一个字节的下一个位置
